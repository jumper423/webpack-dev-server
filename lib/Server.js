...
function Server(compiler, options) {
		...
		proxy: function() {
			if(options.proxy) {
				...
				options.proxy.forEach(function (proxyOptions) {
					proxyOptions.ws = proxyOptions.hasOwnProperty('ws') ? proxyOptions.ws : false;

					if (proxyOptions.ws) {
						this.addWebsocketProxy(proxyOptions);
					} else {
	...
	this.listeningApp.on('upgrade', function(req, socket, head) {
		// Upgraded ws should be proxied too

		this._wsProxies.forEach(function(proxyOptions) {
			// TODO: store output of re
			var re = pathToRegexp(proxyOptions.path);

			// don't include qs when matching url
...
Server.prototype.addWebsocketProxy = function(proxyOptions) {
	this._wsProxies.push(proxyOptions);
}
